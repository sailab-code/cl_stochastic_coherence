...
from lve.metrics_ww import MetricsContainer
...
class VProcessor:
    ...
    def __init__(self, input_stream, output_stream, worker, model_folder, visualization_port=0,
        ...
        if self.worker.options['metrics'] is not None:
            ...
            self.metrics_container = MetricsContainer(num_classes=self.worker.options['net']['supervised_categories'],
    ...
    def process_video(self, log_dict=None):
        ...
        while status[0] == 1:
            ...
            if batch_index == 0:
                ...
                # if cur_frame_number == 301:
                #     with profiler.profile(with_stack=True, record_shapes=True) as prof:
                #         with profiler.record_function("model_inference"):
                #             self.worker.process_frame(batch_img, of=batch_of, supervisions=batch_supervisions, foa=batch_foa)
                #     print(prof.key_averages(group_by_stack_n=True).table(sort_by="cpu_time_total", row_limit=-1))
                #     prof.export_chrome_trace("trace300.json")
                #     prof.export_stacks("stacks300", "self_cpu_time_total")
                # else:
                #     self.worker.process_frame(batch_img, of=batch_of, supervisions=batch_supervisions, foa=batch_foa)
            ...
            # metrics computation
            if self.output_stream.get_output_elements()["sup-probs"]["data"] is not None and \
                    self.worker.options["metrics"] is not None and \
            ...
            # if cur_frame_number == 224:
            #     sys.exit(0)

            # eventually logging on a python list some output elements that could be needed
            if log_dict is not None:
                if log_dict['element'] in self.output_stream.get_output_elements():
    ...
    def load(self):
        ...
        # loading metrics
        self.metrics_container.load(self.model_folder)
